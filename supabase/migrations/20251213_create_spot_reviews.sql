...
-- ポリシー：自分のレビューのみ作成・更新・削除可能
CREATE POLICY "Users can insert their own reviews" ON spot_reviews
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own reviews" ON spot_reviews
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own reviews" ON spot_reviews
